...
for c in $(seq 1 $stTest);
do
	echo "$c test"

	inputFile="DN03_javniTestiPoskus/vhod$(printf "%02d" $c).txt"
	outputFile="DN03_javniTestiPoskus/izhod$(printf "%02d" $c).txt"
	python GeneratorTestnihPrimerov.py > $inputFile
